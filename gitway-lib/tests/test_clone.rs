...
//! Integration tests — end-to-end `git clone` via `GIT_SSH_COMMAND=gitssh`.
//!
//! These tests exercise the full transport relay (FR-14 through FR-17) by
//! cloning a real GitHub repository through the `gitssh` binary.
//!
//! **Prerequisites:**
//! - The `gitssh` binary must be built (`cargo build`).
//! - A valid SSH identity key must be accessible.
//! - Network access to `github.com:22` must be available.
//!
//! Gate with the `GITSSH_INTEGRATION_TESTS` environment variable:
//!
//! ```shell
...
/// Returns the path to the `gitssh` debug binary in the Cargo target directory.
///
...
fn gitssh_binary() -> PathBuf {
    // CARGO_MANIFEST_DIR points to gitssh-lib; walk up one level to the
...
#[test]
fn git_clone_via_gitssh_succeeds() {
    if !integration_enabled() {
        return;
    }

    let binary = gitssh_binary();
    assert!(
        binary.exists(),
        "gitssh binary not found at {}; run `cargo build` first",
        binary.display()
    );
    ...
    assert!(
        status.success(),
        "git clone via gitssh must exit with code 0, got: {status}"
    );
    assert!(
