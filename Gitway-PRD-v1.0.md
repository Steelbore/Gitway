...
| 0.5.0         | **✅ Shipped 2026-05-04.** §5.8.3 (partial) — `anvil_ssh::cert_authority` module + `parse_known_hosts` (FR-60, FR-64); `anvil_ssh::hostkey::host_key_trust` API combining embedded fingerprints + direct pins + matching `@cert-authority` + matching `@revoked` from one `known_hosts` pass; `@revoked` enforcement in `check_server_key` as a policy-overriding blocklist. **FR-61, FR-62, FR-63 deferred** — russh 0.59's `Preferred::DEFAULT.key` excludes `*-cert-v01@openssh.com` host-key algorithms, so server-side host certificates are never surfaced to `check_server_key`; live cert validation lands once russh upstream supports cert host-keys (see §10). |
...
| 0.7.0–0.9.0   | ⏳ In progress (M16+). §5.8 modules added incrementally — one minor per Gitway milestone M16–M19. |
...
| **M14** | **§5.8.3 — `@cert-authority` host CA (partial — FR-60, FR-64)** | `anvil_ssh::cert_authority` module shipped in `anvil-ssh = "0.5.0"`: `parse_known_hosts(&str) -> Result<KnownHostsFile, AnvilError>` + `CertAuthority`, `RevokedEntry`, `DirectHostKey`, `KnownHostsFile` public types; markers recognized case-insensitively, comma-separated host patterns split into multiple entries, OpenSSH-format pubkeys parsed via `ssh_key::PublicKey::from_openssh` for fingerprint computation. New `anvil_ssh::hostkey::host_key_trust(host, custom_path) -> HostKeyTrust { fingerprints, cert_authorities, revoked }` API combines embedded fingerprints + direct pins + matching `@cert-authority` + matching `@revoked` from one `known_hosts` pass; reuses `ssh_config::lexer::wildcard_match` from M12. **`@revoked` enforcement in `check_server_key`** (FR-64): revoked fingerprints checked first, before the `StrictHostKeyChecking::No` bypass — no policy can override. Gitway `gitway config show --json` and `gitway --test --json` gain `cert_authorities: [{host_pattern, algorithm, fingerprint}, ...]` and `revoked: [{host_pattern, fingerprint}, ...]` audit-log keys. **FR-61, FR-62, FR-63 deferred** to a follow-up sub-milestone — russh 0.59's `Preferred::DEFAULT.key` excludes `*-cert-v01@openssh.com` host-key algorithms, so KEX never asks for cert host-keys and `check_server_key` never sees the certificate (see §10). Anvil PRs: [#15](https://github.com/Steelbore/Anvil/pull/15), [#16](https://github.com/Steelbore/Anvil/pull/16), [#17](https://github.com/Steelbore/Anvil/pull/17), [#18](https://github.com/Steelbore/Anvil/pull/18). Gitway PRs: [#24](https://github.com/Steelbore/Gitway/pull/24). Gitway tag: `v1.0.0-rc.5`. | 🟡 Partial 2026-05-04 |
